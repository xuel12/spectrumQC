...
app.layout = html.Div([
    ...
    html.Div(id='spectrum quality training', style={'whiteSpace': 'pre-line'}),  # add a section to store and display output
    html.H2("Prediction"),
    ...
    dcc.Graph(id='important-features-graph',figure = {
            'data': [
                {
                    'x': list(range(2001)), 
                    'y': [random() for i in range(1001)]+[random()/100 for i in range(1002,2001)], 
                    'type': 'bar'
                }
            ],
            'layout': {
...
def update_output_train(n_clicks, value):  # define the function reaching output from input
    if n_clicks != 0:
        ...
        BASE_PATH = '/Users/xuel12/Documents/MSdatascience/DS5500datavis/project1/'
        ...
        param_grid = {'rf': {"min_samples_leaf": [2], "min_samples_split": [5], "n_estimators": [50]}, \
                      'svm': {"kernel": ['linear']}}
        final_model, feature_scores, auc_score = training.modelling_spectrum_quality(temp_dir, model_dir, method='rf', param_grid=param_grid)
        ...
        figure = {'data':[dict(x=feature_scores.index.tolist(),y=feature_scores.tolist(),type='bar')],
                  'layout': {'title': 'important features distribution'}
                  }
        text = 'AUC score of training process is "{}"'.format(auc_score)
    else:
        figure = {'data': [
                {
                    'x': list(range(2001)), 
